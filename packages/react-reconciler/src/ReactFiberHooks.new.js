...
import {now, linkStack, type StackContainer} from './Scheduler';
import {
  trackUsedThenable,
...
export function renderWithHooks<Props, SecondArg>(
  ...
  let children;
  if (__DEV__ && workInProgress._debugStack) {
    children = workInProgress._debugStack.run(Component.bind(null, props, secondArg));
  } else {
...
function dispatchReducerAction<S, A>(
  ...
  const update: Update<S, A> = {
    lane,
    action,
    stack: linkStack('reducer.dispatch'),
    hasEagerState: false,
    eagerState: null,
    next: (null: any),
  };
